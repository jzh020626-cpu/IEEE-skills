...
// Stops at institutional SSO / CARSI pages and never handles credentials.
// Verification challenges are always handed back to the user in the open tab.
...
async function downloadDoi(proxy, doi, outDir, wantSi, debug, { cleanBundle = false, bundleTitle = "" } = {}) {
  ...
  try {
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][doi] verification handoff: ${wall.status} "${wall.reason}"\n`);
      return {
        doi,
        status: wall.status,
        url: info.url,
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (w) {
        if (debug) process.stderr.write(`[debug][doi] verification handoff: ${w.status} "${w.reason}"\n`);
        return {
          doi,
          status: w.status,
          url: meta.url,

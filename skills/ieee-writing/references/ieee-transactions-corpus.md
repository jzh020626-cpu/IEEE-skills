# IEEE Transactions 2025 — Empirical Drafting Patterns (CS/AI corpus)
...
20 open-access *IEEE Transactions* articles in computer science / AI
...
> `ieee-polishing/references/published-article-patterns.md`. This file adds
...
- Four recurring shapes, chosen by intent:
  - Noun phrase, method-led (most common): *Distributed predictive control for communication-constrained autonomous transport*
  ...
  - `System name: function` colon form (brands a tool/model): *FreshTrack: latency-aware coordination for cooperative mobile robots*
...
- Prepositional chain carries "what + how + where": *... coordination and control **with** event-triggered policies **for** industrial mobile robots*.
...
Pattern markers: gap *"However, existing schedulers degrade when wireless contention induces stale state estimates."*; hinge *"Here, we show that model-based coordination outperforms purely reactive policies under packet loss."*; close *"Our findings suggest..."* / *"These results indicate..."*.

## 3. Introduction

- **Hook**, three forms: importance declaration (*"Networked autonomy is central to modern industrial logistics..."*); definition framing (*"Communication-aware control is a joint sensing, networking, and decision problem..."*); domain-then-obstacle (benchmark default: broad use -> *"Despite its promises, progress is impeded by the absence of standardized stress tests."*).
- **Gap** signal words cluster tightly: **However / remains / Unfortunately / underexplored / the scarcity of ... hinder / Without X, Y cannot be ...**. High-end move: quantify the scarcity with domain evidence, e.g., the number of public datasets, baselines, trials, or operating regimes.
...
- **Subheadings** are either a conclusion sentence or a noun/gerund phrase naming the method - *"The proposed scheduler reduces stale execution under bursty contention"*, *"Evaluating robustness across delayed multi-robot transport tasks"*. Avoid neutral *Experiment 1 / Dataset* labels.
- **Conclusion-first**: the paragraph opens with the judgement, evidence and figure call follow - *"The proposed controller maintained stability under the highest tested delay regime..."*
- **Figure callouts**, two shapes: figure-led (*"Figure 3(a) depicts the action distributions..."*) and trailing parenthetical (*"...reduces completion time under high load (Fig. 2(c))"*).

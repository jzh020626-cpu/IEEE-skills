# IEEE Transactions Figure Design Theory

Derived from scripts in the [figures4papers](https://github.com/ChenLiu-1996/figures4papers) repository
and adapted for IEEE-style machine intelligence, robotics, automation, control, and communications figures.
...
- **IEEE Transactions standard**: `font.family = 'sans-serif'`, `font.sans-serif = ['Arial']`
...
When targeting the final dimensions of a two-column `IEEE Transactions` figure page, start smaller than
...
- Keep only left + bottom spines — minimalist, IEEE Transactions-approved.
...
3. Reserve green/red for arrows, gains, drops, thresholds, or signed engineering direction.
...
### Modality-specific palette discipline from sampled 2026 IEEE Transactions figures
...
- **Reliability/deployment composites**: dark baseline/reference series, restrained warm/cool follow-up hues,
  pale background bands in forest plots.
- **Robotics / systems pages**: neutral grey scaffolds plus a small number of engineeringly
...
### IEEE Transactions page archetypes from sampled 2026 papers

`IEEE Transactions` figures were not uniformly dashboard-like. They repeatedly used a few strong page
archetypes:
...
| Reliability/deployment triptych | Top time-series row, middle interval/effect row, bottom summary row | Reuse the same column logic across outcomes and put the shared legend above the row |
...
- For reliability or longitudinal triptychs, place one shared legend above the row rather than repeating it per axis.
...
- [ ] Panel c adds a dimension absent from a and b (e.g., correlation, engineering relationship)
...
| Subset of parent | Failure-only ranked bar is just one column of the stacked bar | Swap for scatter: failure rate vs. throughput |
...
```python
# heat: DataFrame (operating regimes × metric categories), values in %
z = (heat - heat.mean(axis=0)) / heat.std(axis=0)
im = ax.imshow(z.values, cmap="RdBu_r", aspect="auto", vmin=-2.5, vmax=2.5)
# colorbar label:
cbar.set_label("Z-score vs operating-regime mean")
```
...
```python
# x: dominant operating regime (e.g., packet loss %)
# y: functional readout (e.g., completion success %)
# size: third variable (e.g., delay variance)
ax.scatter(x, y, s=delay_variance * scale, c=colors,
...
Label quadrants ("low loss / high success", "high loss / low success", etc.) with small grey text.

---

## 10) Reproduction Checklist

To match IEEE Transactions publication standards:

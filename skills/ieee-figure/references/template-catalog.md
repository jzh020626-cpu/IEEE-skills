...
|---|---|---|
| `volcano` | feature label, effect-size, adjusted-p columns | positive p-value check, explicit thresholds, all points retained, large-data rasterization |
...
```bash
python skills/ieee-figure/scripts/plot_templates.py volcano \
  --input sensitivity.csv --label-col parameter --effect-col effect --p-col q_value \
  --effect-label "Standardized effect" --output figures/effect_significance

python skills/ieee-figure/scripts/plot_templates.py roc \
  --input roc.csv --fpr-col fpr --tpr-cols model_a,model_b \
  --output figures/roc

python skills/ieee-figure/scripts/plot_templates.py dotplot \
  --input benchmark_matrix.csv --row-col method --column-col metric \
  --size-col coverage --color-col normalized_score --output figures/benchmark_matrix
```

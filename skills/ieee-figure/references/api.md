# API Reference — IEEE Transactions Figure Making
...
def apply_publication_style(font_size=16, axes_linewidth=2.5, use_tex=False):
    """Apply IEEE Transactions-style rcParams. Call once before creating any figures."""
...
def add_panel_label(ax, label, x=-0.06, y=1.02, fontsize=14,
    ...
    """Place a IEEE Transactions-style panel label near the top-left edge."""
    ax.text(
        x, y, label,
...
def make_forest_plot(ax, labels, estimates, ci_low, ci_high,
    ...
    """
    Minimal interval plot helper for IEEE Transactions-style reliability/statistical panels.
    """
    import numpy as np
...
reliability-triptych look from `IEEE Transactions`.

...
- Translate intent, not wording. Chinese phrases such as "可向通讯作者索取" are usually too vague for IEEE Transactions-style English unless the restriction and access process are specified.
- Convert Chinese repository/status descriptions into precise publication terms:
  `数据可用性声明` -> `Reproducibility Package`; `原始数据` -> `raw data`; `处理后数据` -> `processed data`; `源数据` -> `figure/table data`; `补充材料` -> `supplementary material`; `受限数据` -> `restricted data`; `合理请求` -> `reasonable request`, only with reason and review route.

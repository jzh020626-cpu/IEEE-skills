#!/usr/bin/env bash
# IEEE Academic Search Skill + MCP Server Installer for Claude Code
...
echo "=== IEEE Academic Search Installer ==="
...
if [ "${#missing_paths[@]}" -gt 0 ]; then
    echo "ERROR: incomplete ieee-academic-search skill directory." >&2
    echo "Missing required path(s):" >&2
    for relpath in "${missing_paths[@]}"; do
        echo "  - ${SCRIPT_DIR}/${relpath}" >&2
    done
    echo "Install the full skills/ieee-academic-search folder and retry." >&2
    exit 1
fi
...
echo "  2. Optionally configure Scopus or ScienceDirect credentials for broader engineering discovery"
echo "  3. Test: ask Claude 'search IEEE Transactions papers about networked control under packet loss'"
echo

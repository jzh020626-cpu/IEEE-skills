...
When available, route broad discovery and citation verification through `ieee-academic-search`, and lawful full-text retrieval through `ieee-downloader`. Run independent source searches in parallel when permitted, then deduplicate by DOI, title, and study identity.
...
Route submission-grade scientific figures through `ieee-figure` when available. Match the production method to the evidence type:
...
After the framework passes review, use `compose-mode.md` for section drafting, `review-critique-methodology.md` for critical-depth QA, and `specialist-lens-review.md` for targeted specialist review.

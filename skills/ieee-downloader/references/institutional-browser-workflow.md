...
When a publisher verification page appears, stop automated actions on that tab. Record `publisher_verification_waiting_user` or the provider-specific challenge status, tell the user which paper and tab need manual attention, and continue from the same tab only after the user confirms completion. If verification immediately reappears, mark `do_not_auto_retry` and move on. Do not automate CAPTCHA, slider, Turnstile, reCAPTCHA, or comparable anti-bot controls.
...
4. There is no visible QR-only login, SMS/OTP field, CAPTCHA or anti-bot control, push-approval prompt, password reset prompt, consent-to-share-new-data prompt, or account/security warning.
...
& $node "$env:USERPROFILE\.agents\skills\ieee-downloader\scripts\browser_pdf_downloader.mjs" `

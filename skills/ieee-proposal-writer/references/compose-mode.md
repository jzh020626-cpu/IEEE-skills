...
**Rule**: only ask questions whose answers change the writing path. If the user says "write a proposal about safe multi-robot control" and context already identifies a Chinese doctoral plan covering the first project stage, do not re-ask known document metadata; state the bounded assumption and proceed.
...
**Process**: run one or two independent specialist lenses (doctoral proposal reviewer + domain expert). Score foundations using evaluation rubric. If `foundation_score < 7.5`, return to the weakest foundation file and fix it before drafting.

**Output**: specialist-lens review notes in `qa_logs/foundation_review.md`, updated foundation files if issues found.

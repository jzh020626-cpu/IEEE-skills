...
| `SOFTEN_CLAIM` | Reduced claim strength or added boundary | The original claim was too broad, causal, novel, deployment-level, or mechanistic |
...
| `OUT_OF_SCOPE` | Valid suggestion but outside current manuscript scope | The request requires a new platform, system, long-horizon validation, or different study |
...
- If the author says only "we revised it", use `AUTHOR_INPUT_NEEDED` until the location and ieee of the revision are known.

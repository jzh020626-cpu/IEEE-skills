...
run_update "$TMP_DIR/second-codex-skills"

stamp_count=$(find "$TMP_DIR/state/ieee-skills" -name last-check -type f | wc -l | tr -d ' ')
...
state_dir=$(find "$TMP_DIR/concurrent-state/ieee-skills" -mindepth 1 -maxdepth 1 -type d | head -n 1)

...
```text
The [raw/processed/figure-table] data supporting the findings of this study are available in
[Repository] under accession [ACCESSION] / at [DOI or persistent URL]. The deposited record
contains [brief contents: e.g. raw measurements, processed tables, figure/table data, metadata
and analysis inputs].
```
...
and figure/table data in [Repository/supplementary file] under [identifier or file name].
```
...
figure/table data, or supplementary files.

```text
All data supporting the findings of this study are included in the paper and its
supplementary material. [Name exact supplementary tables, logs, scripts, or data files when possible.]
```

中文对应：只有当支撑结论的数据确实都在正文和补充材料中时才这样写。
...
identifier/version]. Figure/table data for [figures/tables] are provided in [location].
```
...
metadata, synthetic data, or figure/table data] in [location] to support interpretation of the results.
```
...
subject to [approval, agreement, or legal condition]. [Public metadata/aggregate data/figure-table data]
...
| All data are in the manuscript. | Often false for figures/statistics. | Name exact figure/table data, supplementary files, and omitted raw data. |
| Data are proprietary. | Does not say who controls access. | Name owner/provider and access route. |
| N/A. | IEEE Transactions-style instructions usually require an explanation. | State why no datasets were generated or analysed. |

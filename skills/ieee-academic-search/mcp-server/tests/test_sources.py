...
class TestCrossRefSearch:
    ...
    def test_get_by_doi(self, mock_get, mock_config):
        ...
        mock_resp.json.return_value = {
            "message": {
                ...
                "container-title": ["IEEE Transactions on Example Systems"],
                ...
                "references-count": 42,
                "URL": "https://doi.org/10.1109/example.2024.1234567",
            }
        }
        mock_get.return_value = mock_resp
        ...
        source = CrossRefSource()
        detail = source.get_by_doi("10.1109/example.2024.1234567")

        assert detail["title"] == "A Great Paper"
        assert detail["doi"] == "10.1109/example.2024.1234567"
    ...
    def test_get_citation(self, mock_get, mock_config):
        ...
        mock_resp.status_code = 200
        mock_resp.raise_for_status = MagicMock()
        mock_resp.text = "Smith, A. (2023). Deep Learning. IEEE Transactions."
        mock_get.return_value = mock_resp

        from sources.crossref import CrossRefSource

        source = CrossRefSource()
        citation = source.get_citation("10.1038/ieee12373", style="ieee")
...
class TestPubMedSearch:
    ...
    def test_search_esearch_efetch_flow(self, mock_get, mock_config):
        ...
        <PubmedArticleSet>
            <PubmedArticle>
                <MedlineCitation>
                    <PMID>99999999</PMID>
                    <Article>
                        <ArticleTitle>Robotic Analysis of Cancer</ArticleTitle>
                        <AuthorList>
                            <Author>
                        ...
                        <Journal>
                            <Title>IEEE Transactions Medicine</Title>
                            <JournalIssue>
        ...
        source = PubMedSource()
        result = source.search("cancer robotics", rows=5)

        assert result["total"] == 1
        assert result["query"] == "cancer robotics"
        assert len(result["results"]) == 1

        item = result["results"][0]
        assert item["title"] == "Robotic Analysis of Cancer"
        assert item["authors"] == ["Wang Li", "Zhang Wei"]
        assert item["year"] == 2024
        assert item["pmid"] == "99999999"
        assert item["doi"] == "10.1038/s41591-024-00001"
        assert item["journal"] == "IEEE Transactions Medicine"
...
class TestDetectIdType:
    ...
    def test_detect_doi(self):
        ...
        assert _detect_id_type("10.1109/5.771073") == "doi"
        assert _detect_id_type("10.1109/TAC.2024.000000") == "doi"
        assert _detect_id_type("10.1109/TII.2025.000000") == "doi"

    def test_numeric_id_not_default_ieee_identifier(self):
        from academic_search_server import _detect_id_type

        with pytest.raises(ValueError, match="Cannot detect"):
            _detect_id_type("12345678")
        with pytest.raises(ValueError, match="Cannot detect"):
            _detect_id_type("1234567")
    ...
    def test_detect_short_number_raises(self):
        """Bare numeric IDs are not part of the default IEEE identifier set."""
        from academic_search_server import _detect_id_type
...
class TestResolveIdType:
    ...
    def test_explicit_doi(self):
        ...
        assert _resolve_id_type("10.1109/test", "doi") == "doi"

    def test_explicit_numeric_biomedical_id_unsupported(self):
        from academic_search_server import _resolve_id_type

        with pytest.raises(ValueError, match="Unsupported"):
            _resolve_id_type("12345678", "pmid")

    def test_auto_delegates(self):
        from academic_search_server import _resolve_id_type

        assert _resolve_id_type("10.1109/test", "auto") == "doi"

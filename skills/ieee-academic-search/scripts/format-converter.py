# -*- coding: utf-8 -*-
"""
IEEE-oriented citation downloader with format conversion.
Default sources: CrossRef (REST API) and arXiv (Atom API). Optional PMID lookup
is retained for explicit user requests only.
Outputs: .ris, .bib, .enw, and optional .nbib for PMID records.

Usage:
  python format-converter.py --doi 10.1109/5.771073 --format bib
  python format-converter.py --doi 10.1109/TAC.2024.000000 --format ris
  python format-converter.py --arxiv 2401.00000 --format ris
...
def main():
    parser = argparse.ArgumentParser(
        description="IEEE-oriented citation downloader with format conversion (.ris/.bib/.enw)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --doi 10.1109/5.771073 --format bib
  %(prog)s --doi 10.1109/TAC.2024.000000,10.1109/TII.2025.000000 --format ris
  %(prog)s --arxiv 2401.00000 --format bib
  %(prog)s --query "networked control packet loss stability"
...
refs.txt format:
  ...
  # Lines starting with # are comments
    ...
    parser.add_argument("--author", help="Author name for optional PMID search")
    parser.add_argument("--title", help="Title keywords for optional PMID search")
    parser.add_argument("--query", help="Optional PMID search query; for IEEE discovery prefer academic_search.py")
    parser.add_argument("--input", help="Input file with references")
    parser.add_argument("--interactive", action="store_true", help="Interactive mode")
    parser.add_argument(
        "--format", choices=["nbib", "ris", "bib", "enw"], default="ris",
        help="Output format: ris (default, EndNote/Zotero), bib (BibTeX/LaTeX), enw (EndNote tagged), nbib (PMID only)",
    )
    parser.add_argument(

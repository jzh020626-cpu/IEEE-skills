...
portable, path-independent, and designed for the ieee-figure contract.
"""
...
def demo_rows(kind: str) -> list[dict[str, Any]]:
    ...
    if kind == "volcano":
        ...
        for index in range(1200):
            ...
            rows.append({"feature": f"Feature_{index + 1}", "effect": effect, "q_value": pvalue})
        return rows
    if kind == "roc":
        rows = []
    ...
    if kind == "dotplot":
        rows = []
        row_names = ["Baseline", "Method A", "Method B", "Proposed"]
        column_names = ["Accuracy", "Latency", "Energy", "Memory", "Robustness", "Reliability"]
                ...
                rows.append(
                    {
                        "method": row_name,
                        "metric": column_name,
                        "coverage": float(np.clip(70 - 14 * distance + rng.normal(0, 5), 1, 100)),
                        "normalized_score": float(np.clip(1 - distance / 6 + rng.normal(0, 0.08), 0, 1)),
                    }
                )
        return rows
    if kind == "marginal":
    ...
    if kind == "paired":
        ...
        for index in range(28):
            ...
            rows.extend(
                [
                    {"run": f"R{index + 1:02d}", "condition": "Baseline", "value": before},
                    {"run": f"R{index + 1:02d}", "condition": "Proposed", "value": after},
                ]
            )
        return rows
...
def plot_volcano(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    required = [args.label_col, args.effect_col, args.p_col]
    ...
    labels = [str(row[args.label_col]) for row in rows]
    ...
    qa["mapping"] = {"label": args.label_col, "effect": args.effect_col, "adjusted_p": args.p_col}
...
def plot_dotplot(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    cmap = LinearSegmentedColormap.from_list("ieee_metric", ["#2166AC", "#F7F7F7", "#B2182B"])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_common_arguments(volcano, 88.9, 82.0)
    volcano.add_argument("--label-col", "--gene-col", dest="label_col", default="feature")
    volcano.add_argument("--effect-col", default="effect")
    volcano.add_argument("--p-col", default="q_value")
    volcano.add_argument("--effect-label", default="Effect size")
    ...
    add_common_arguments(roc, 88.9, 88.9)
    ...
    dotplot = subparsers.add_parser("dotplot", help="size/color matrix dot plot")
    add_common_arguments(dotplot, 182.0, 105.0)
    dotplot.add_argument("--row-col", default="method")
    dotplot.add_argument("--column-col", default="metric")
    dotplot.add_argument("--size-col", default="coverage")
    dotplot.add_argument("--color-col", default="normalized_score")
    ...
    dotplot.add_argument("--size-label", default="Coverage (%)")
    dotplot.add_argument("--color-label", default="Normalized score")

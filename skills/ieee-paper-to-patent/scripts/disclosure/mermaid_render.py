...
def _find_mmdc_invocation() -> tuple[list[str], bool]:
    ...
    Windows 优先使用 ``npx.cmd``；若只找到 ``npx.ps1``，则通过显式
    PowerShell 参数数组调用，避免拼接 shell 命令。
    ...
    if sys.platform == "win32":
        ...
        if not npx:
            raise RuntimeError("npx was not found; install Node.js or a local Mermaid CLI")
        if Path(npx).suffix.lower() == ".ps1":
            powershell = shutil.which("pwsh.exe") or shutil.which("powershell.exe")
            if not powershell:
                raise RuntimeError("npx resolves to PowerShell, but no PowerShell executable was found")
            return [
                powershell,
                "-NoProfile",
                "-NonInteractive",
...
def _render_one_mermaid(
    ...
    *,
    ...
    try:
        ...
        if use_shell:
            raise RuntimeError("shell-based Mermaid execution is disabled")
        cmd = [
            *mmdc_base,
            "-i",
            str(tmp_path),
...
def render_markdown_mermaid(
    ...
    *,
    ...
    mmdc_base: list[str] = []
    use_shell = False
    mmdc_setup_error: Exception | None = None
    try:
        mmdc_base, use_shell = _find_mmdc_invocation()
    except Exception as exc:
        mmdc_setup_error = exc
    ...
    while i < len(lines):
        ...
        if _MMD_START.match(line):
            ...
            try:
                if mmdc_setup_error is not None:
                    raise mmdc_setup_error
                _render_one_mermaid(
                    "".join(body),
                    png_path,
                    mmdc_base,

...
NumPy-like data, or custom layout, and do not infer R just because the task is an engineering
audit with tabular statistics.
...
|---|---|
| The user brings R scripts, RData/RDS, CSV summaries, reliability intervals, or ggplot templates | The data pipeline is already Python, NumPy/Pandas arrays, PyTorch/TensorFlow outputs, image arrays, ROS bags, or simulation output |
| The target plot is `ggplot2`, `patchwork`, `ComplexHeatmap`, `circlize`, `forestplot`, or R-heavy tabular summaries | The target plot needs low-level custom layout, Matplotlib patches, image plates, subplot mosaics, robot trajectories, network diagrams, or custom drawing primitives |
...
| Heatmap annotations are engineeringly rich and multi-layered | Image panels and quantitative panels need tight pixel/axis control |
...
- Reliability/deployment statistics: `forestplot`, `ggplot2`
- Multi-robot/communication: `ROS bag`, `NetworkX`, `ComplexHeatmap`
...
1. Export clean figure/table data as CSV/TSV with stable column names.
2. Assemble the final figure in the selected backend.
3. Keep the figure/table data file next to the plotting script.
...
multi-layer reliability annotation and patchwork assembly. I will still keep the export contract

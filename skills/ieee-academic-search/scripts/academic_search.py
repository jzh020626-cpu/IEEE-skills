...
# ///
"""No-MCP fallback search for ieee-academic-search (OpenAlex, stdlib only).
...
`ieee-citation/scripts/ieee_citation.py` already does for CrossRef: a single
...
CrossRef and arXiv-deposited works, so one endpoint covers many journals,
preprints and discovery sources. Pair it with
`format-converter.py` to download/convert the chosen DOIs/arXiv IDs into
.ris/.bib/.enw citation files — together they make the search→export workflow
runnable without MCP.

Usage:
    # By topic
    python3 academic_search.py "networked control packet loss stability" [--limit 10] [--year-from 2020] [--sort cited_by_count|relevance_score|publication_date]
    ...
    python3 academic_search.py "distributed control" --author "Wanshui Han" --sort publication_date
...
MAILTO = "ieee-trans-skills@users.noreply.github.com"
...
# ultra-cited papers from unrelated fields for an engineering query
...
def _cluster_summary(results: list[dict]) -> list[dict]:
    ...
    unrelated same-name clusters instead of silently

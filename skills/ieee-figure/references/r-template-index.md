...
- When a template is useful, describe it generically by chart family: "a grouped bar
  template", "a ComplexHeatmap workflow", "a reliability interval plotting workflow".
...
| PCA, PCoA, NMDS, tSNE, UMAP | `prcomp`, `cmdscale`, `vegan`, `Rtsne`, `ROS bag`, embedding coordinates |
| Reliability, subgroup, calibration, interval, forest | `forestplot`, `geom_pointrange`, `ggplot2`, confidence intervals, reliability ratios |
| Benchmark and component summaries | grouped metrics, ablation tables, dot plots, ridge plots |
| Circular or chord-style systems summaries | `circlize`, network adjacency, communication links, resource allocation |
| Multi-robot and communication workflows | `ROS bag`, trajectory metrics, latency sensitivity, robot-role annotation |
...
- Ensure each statistical comparison has `n`, center, spread, test, and correction
  information in the legend or figure/table data notes.

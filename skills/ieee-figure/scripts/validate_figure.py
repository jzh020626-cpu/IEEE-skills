...
def check_dimensions(source: str, backend: str) -> Finding:
    ...
    width = widths[0]
    if abs(width - 88.9) <= 4 or abs(width - 182.0) <= 6:
        return finding("FINAL-WIDTH", "PASS", f"Detected width {width:.1f} mm matches a common IEEE journal column width")
    return finding(
        "FINAL-WIDTH",
        "WARN",
        f"Detected width {width:.1f} mm is not near the common IEEE 88.9/182 mm defaults; verify the target journal",
    )

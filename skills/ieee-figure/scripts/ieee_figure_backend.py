#!/usr/bin/env python3
"""Read or write the user's default ieee-figure plotting backend."""
...
def config_path() -> Path:
    ...
    return Path("~/.config/ieee-skills/ieee-figure.json").expanduser()
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Invalid ieee-figure config JSON at {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise SystemExit(f"Invalid ieee-figure config at {path}: expected object")
    return data

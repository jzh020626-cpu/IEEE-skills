...
- Scope of authority:
  - publication criteria for `IEEE Transactions` Articles
...
- Editor-versus-referee boundary in the source:
  ...
  - editors are `not strictly bound` by referee editorial opinions on whether the work belongs in `IEEE Transactions`
...
- The output includes an explicit `Risk / unsupported claims` section.
  - This is a local QA device, not an official IEEE Transactions format requirement.
...
- If the user asks for an author rebuttal, route to `ieee-response`, not this skill.

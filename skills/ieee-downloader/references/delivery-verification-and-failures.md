...
```powershell
$env:PYTHONUTF8='1'
python -X utf8 "$env:USERPROFILE\.claude\skills\ieee-downloader\scripts\extract_pdf_text.py" `
...
If direct publisher navigation triggers ScienceDirect "Are you a robot?", Cloudflare, CAPTCHA, or another bot challenge, stop automation, record the provider-specific waiting status, ask the user to complete it in Chrome, and continue once from the same page after confirmation. If it immediately reappears, mark `do_not_auto_retry`. Do not use anti-bot automation.

...
- The core conclusion should be one sentence with a verb: "Controller X reduces
  tracking error under delayed feedback", not "Controller results".
...
| `schematic-led system figure` | A workflow, system model, device, control loop, or experimental design must be understood first | Left or top schematic, 35-60% of area | 2-4 quantitative validation panels |
| `experiment snapshot + quant` | Robot photos, simulator scenes, maps, sensor frames, or state overlays lead the evidence | Representative setup/snapshot | Scale/coordinate/task overlays, quantification |
...
1. Establish the system: task, method, platform, device, or experimental design.
...
colors, symbols, workflow direction, task classes, and scale. Reuse that vocabulary
...
- Is the trial/seed count visible in the legend or figure/table data?
- Are error bars, intervals, and statistical tests defined?
- Are axes comparable across panels that invite comparison?
- Are representative snapshots quantified and traceable to raw logs or source files?
- Are screenshot/image adjustments documented and non-misleading?

...
Use this file when the user writes in Chinese, provides a Chinese Reproducibility Package draft, or asks
for bilingual wording. The goal is not to translate Chinese literally. The goal is to convert the
author's Chinese description into a IEEE Transactions-ready English availability route.

## Core terminology

| 中文 | Preferred English | Notes |
|---|---|---|
| 数据可用性声明 / 数据获取声明 | Reproducibility Package | Use the journal heading `Reproducibility Package`. |
...
| 源数据 | figure/table data | Data underlying figures or tables. |
| 补充材料 / 附录 | supplementary material | Use exact file/table names when possible. |
...
- Convert "本文所有数据均包含在正文和补充材料中" to a specific claim:
  name figure/table data files, supplementary tables, logs, scripts, or repository records. If raw data are absent, say
...
5. 是否有代码、脚本或 README 能解释 raw data 到 figure/table data 的处理过程？
```

## Common Chinese draft fixes

| 中文原意 | Avoid literal English | IEEE Transactions-ready direction |
|---|---|---|
...
| 所有数据见补充材料。 | All data are in the supplementary materials. | Name exact supplementary tables, logs, scripts, and figure/table data; flag missing raw data if any. |

...
async function main() {
  ...
  const initJs = `(
    async () => {
      ...
      window.__ieeeDownloaderBytes = new Uint8Array(ab);
      return {
        ok: r.ok,
        status: r.status,
        contentType: ct,
        size: window.__ieeeDownloaderBytes.length,
        url: location.href,
        head: Array.from(window.__ieeeDownloaderBytes.slice(0, 8))
      };
    }
  )()`;
  ...
  if (!args.allowNonPdf && !isPdfHead(headBytes)) {
    ...
    await proxyEval(args.proxy, target, `delete window.__ieeeDownloaderBytes`, 10000).catch(() => {});
    throw new Error(
  ...
  try {
    for (let start = 0; start < size; start += args.chunkSize) {
      ...
      const chunkJs = `(
        () => {
          const bytes = window.__ieeeDownloaderBytes.slice(${start}, ${end});
          let bin = "";

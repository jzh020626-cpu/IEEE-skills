...
Keep the 16 headings stable, but adapt their contents to the paper type. A review may not have system modules or ablations; a theoretical paper may not have datasets; an application paper may not introduce a new algorithm. Use `Not applicable` with a reason instead of forcing an algorithm-paper template onto every paper.

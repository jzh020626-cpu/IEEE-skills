...
function classifyCnkiWall(url = "", title = "", body = "") {
  ...
  // Slider/drag/puzzle CAPTCHA — always requires user handoff.
  if (/滑块|滑动验证|拖动滑块|拼图验证|请按住滑块|请拖动|slide to verify|drag the slider|slide verification|slider captcha/i.test(text)) {
    return { status: STATUS.PUBLISHER_VERIFICATION_WAITING_USER, reason: "CNKI verification challenge requires user action" };
  }
  // Login/auth wall
...
export async function downloadCnkiTitle(proxy, title, outDir, { cnkiUrl = DEFAULT_CNKI_URL, format = "any", debug = false, wantSi = false, transport = null } = {}) {
  ...
  try {
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][cnki] verification handoff after search: ${wall.status} "${wall.reason}"\n`);
      return {
        title,
        status: wall.status,
        url: snap.url,
        reason: wall.reason,
        nextAction: "user_complete_verification_in_browser",
      };
    }
    ...
    if (wall) {
      if (debug) process.stderr.write(`[debug][cnki] verification handoff after detail navigation: ${wall.status} "${wall.reason}"\n`);
      return {
        title,
        status: wall.status,
        url: snap.url,

...
Unified entry point exposing IEEE-first engineering literature search through
CrossRef, arXiv, and optional configured Scopus/ScienceDirect providers.
"""
...
async def search_papers(
    ...
    Args:
        ...
        sources: List of source names to query. Defaults to CrossRef and arXiv
            for IEEE-first engineering discovery. Add "scopus" and/or "sciencedirect" explicitly to use
    ...
    valid_sources = {"crossref", "arxiv", "scopus", "sciencedirect"}
...
def get_scopus_abstract(
    ...
    Args:
        identifier: EID, Scopus ID, DOI, or PII.
...
@mcp.tool()
def get_citation(id: str, id_type: str = "auto", style: str = "ieee") -> str:
    """Get formatted citation for a paper.

    Uses CrossRef content negotiation for DOI-based citations.
    For arXiv IDs, fetches metadata first then generates a basic citation.

    Args:
        id: Paper identifier (DOI or arXiv ID).
        id_type: Force identifier type ("doi", "arxiv", or "auto").
        style: Citation style. Supported: ieee, apa, harvard,

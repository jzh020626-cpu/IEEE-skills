# IEEE Downloader 重构参考源码研究

日期：2026-07-15

## 研究范围与版本

本笔记只研究用户指定的两个一手源码仓库，不讨论 IEEE Downloader 当前实现，也不修改现有代码。
...
- 该实现没有 DOI、题名、作者、年份的消歧评分，也没有对多个同名结果进行确认；IEEE Downloader 需要独立的规范化文献实体与置信度。
...
- 当前校验存在明显漏洞：若内容既不是 PDF 也不是 CAJ，但大于 10 KB，代码仍会写成 `.pdf` 并标记成功；IEEE Downloader 必须将“HTTP 成功”与“文件有效”严格分开。
...
对 IEEE Downloader 的意义：只应借鉴 OA 子系统，不应把整个数据集构建器嵌入交互式 skill，也不能用它替代非 OA 出版商 API 或用户机构授权。
...
- biblio-glutton 默认公共实例的可靠性连 README 自己也提示不足；它不是 IEEE Downloader 必需依赖。[服务说明](https://github.com/kermitt2/article_dataset_builder/blob/920fe298a81061e8a26d55d13829ccb6deb6501c/Readme.md#L94-L108)
...
- 项目声称 fair-use 覆盖不可再分发文章，但“可下载”不等于“允许再分发”；IEEE Downloader 必须把个人获取、OA 状态与再分发许可分开记录，不能沿用模糊语义。[项目声明](https://github.com/kermitt2/article_dataset_builder/blob/920fe298a81061e8a26d55d13829ccb6deb6501c/Readme.md#L15-L15)
...
对于交互式 IEEE Downloader，应借鉴状态模型和 provenance sidecar，而不是引入整套 LMDB/S3/GROBID 数据集设施。单篇下载可用轻量 manifest；批量模式再考虑 SQLite/队列。
